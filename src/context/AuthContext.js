...
export const AuthProvider = ({ children }) => {
  ...
  // 로그인 메서드
  const login = (newToken, newNickname, newProfileImage, newPlatform, newCreatedAt) => {
    setToken(newToken);
  ...
  return (
    <AuthContext.Provider value={{ isLoggedIn, token, nickname, profileImage, platform, createdAt, login, logout }}>
      {children}
